...
export const PhotosUploader: React.FunctionComponent<PhotosUploaderPropsT> = ({ photos }) => {
  ...
  const handleChangeFirstPhoto = (idx: number) => {
    console.log(idx);
    dispatch(changeFirstPhoto(idx));
  };

  const renderedAddedPhotos = photos.map((photo, i) => (
    <div key={photo.name} className="w-48 h-32 relative">
      <img className="w-full h-full object-cover object-center rounded-2xl" src={photo.url} />
      <div className="absolute bottom-0 right-0">
        <button
          className="text-white bg-black m-2 p-2 rounded-full opacity-70"
          type="button"
          onClick={() => {
      ...
      <div className="absolute bottom-0 left-0">
        <button
          className="text-white bg-black m-2 p-2 rounded-full opacity-70"
          type="button"
          onClick={() => handleChangeFirstPhoto(i)}
        >
          {i === 0 ? (
            <svg
              xmlns="http://www.w3.org/2000/svg"
